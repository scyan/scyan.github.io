...
/*$.prototype.gallery=function(options){
	return this.each(function(i) {
		if (!$(this).data('mutiImg')) {
			$(this).data('mutiImg', new MutiImg(options, $(this)));
		}
		var mutiImg = $(this).data('mutiImg');
		if (typeof options === 'string') {
			if (typeof mutiImg[options] === 'function') {
				(mutiImg[options])();
			}
		}
	});
...
function Gallery(options, $obj) {
	this.options = {
		...
		type:1,//1:完整包括九宫格和滑动gallery,2:只有九宫格,3:只有gallery,4:只有下方滑动
	};
	var config = {
		firstGet:0,
		focus:0,//焦点图片索引
		...
		gallery : $('<div class="gallery hide"></div>'),
		...
		loading:$('<img class="loading_gif" src="./icons/loading.gif"/>')
	}
	var Dispacher={
	...
	function initConfigs(){
		// 判断是否为数组
		if (options && typeof options == 'object'&& typeof options.length == 'number'&& typeof options.splice == 'function'&& !(options.propertyIsEnumerable('length'))) {
			this.options.imgs = options;
		} else if (typeof options === 'object') {
			$.extend(this.options, options);
			$.extend(this.config, this.options);
		}
	...
	function initUI() {
		...
		if(this.options.type==2){//只有九宫格
			config.shell.append(config.squares);
		}else if(this.options.type==3){//只有gallery
			config.shell.append(config.gallery);
			config.gallery.append(config.showBox).append(config.chooseBox).removeClass('hide');
			config.chooseBox.append(config.arrowPre).append(config.stagBox).append(
					config.arrowNext);
			config.showBox.append(config.loading);
		}else if(this.options.type==4){//只有滑动模块
			config.shell.append(config.gallery);
			config.gallery.append(config.chooseBox).removeClass('hide');
			config.chooseBox.append(config.arrowPre).append(config.stagBox).append(
					config.arrowNext);
		}else{
		...
		for ( var i = 0; i < this.options.imgs.length; i++) {
			config.squares.append('<li><img src="' + this.options.imgs[i].src
				//	+ '" pic_id="' + this.options.imgs[i].pic_id
					+ '" alt="'+(this.options.imgs[i].alt?this.options.imgs[i].alt:"")+'"></img></li>');
			$('ul', config.stagBox).append('<li><a href="javascript:void(0);"'
					+ '><img src="' + this.options.imgs[i].src
					+ '" alt="'+(this.options.imgs[i].alt?this.options.imgs[i].alt:"")+'"></img></a></li>')
		}
		...
		Dispacher.connect('arrowListener',function(event,args){
			if(parseFloat($('ul', config.stagBox).css('margin-left')) <= config.stagLength- config.trueLength){
				config.arrowNext.addClass('disabled');
			}else{
				config.arrowNext.removeClass('disabled');
			}
			if(parseFloat($('ul', config.stagBox).css('margin-left'))>=0){
		...
		Dispacher.connect('imgSlid',function(event,args){
			...
			$('ul', config.stagBox).animate({
					"margin-left" : (args.operator?args.operator+'=':'')+args.slid
			}, 500,function(){
	...
	function initActions() {
		...
		config.squares.delegate('img', 'click', function() {
			...
			changeFocus.apply(that, [ $(this).data('index'), function() {
				config.loading.hide();
		...
		config.arrowNext.click(function() {
			if (parseFloat($('ul', config.stagBox).css('margin-left')) > config.stagLength- config.trueLength) {
				var slid = config.stagLength- config.trueLength- parseFloat($('ul', config.stagBox).css('margin-left'));
				if (Math.abs(slid) > config.stagLength) {
					slid = -config.stagLength;
				}
					Dispacher.notify('imgSlid',{operator:'+',slid:slid});
						
				}

		});
		// 前滚
		config.arrowPre.click(function() {
			if (parseFloat($('ul', config.stagBox).css('margin-left')) < 0) {
				var slid = parseFloat($('ul', config.stagBox).css('margin-left'));
				if (Math.abs(slid) > config.stagLength) {
					slid = -config.stagLength;
				}
				Dispacher.notify('imgSlid',{operator:'-',slid:slid});
		...
		var mouseMoveFunc=function(e){
			...
			if (x < $(this).width()/3.0) {
				...
				if(config.focus!=0){
					$(this).removeClass('rightCursor').removeClass('smallCursor').addClass('leftCursor');
				}else{
					$(this).removeClass('rightCursor').removeClass('leftCursor').addClass('smallCursor');
				}
				//console.log(1);
			}else if(x>=$(this).width()/3.0&&x<=$(this).width()*2.0/3.0){
				...
				if(config.focus==config.count-1){
					...
					$(this).removeClass('smallCursor').removeClass('leftCursor').addClass('rightCursor');
				}
				//console.log(3);
			}
		}
		config.showBox.delegate('img', 'click', function(event) {
			...
			if (x < $(this).width() / 3.0) {
				if ($(this).data('index') === 0) {
					...
					changeFocus.apply(that, [ $(this).data('index') - 1 ]);
				}
	...
	function showBigImg(index,callback){
		...
		img.onload=function(){
			...
			$('ul li a', config.stagBox).css('border', 'solid 2px transparent');
			$('ul li:nth-child(' + (index + 1) + ') a', config.stagBox).css(
				'border', 'solid 2px #f00');
		
			// 居中显示
			var left = index * config.cellLength + config.cellLength / 2.0;
			...
			Dispacher.notify('imgSlid',{slid:slid});
		}
	}
